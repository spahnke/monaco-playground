...
export function restartLanguageServer(): void {
	...
	monaco.typescript.typescriptDefaults.setWorkerOptions(monaco.typescript.typescriptDefaults.workerOptions);
	monaco.typescript.javascriptDefaults.setWorkerOptions(monaco.typescript.javascriptDefaults.workerOptions);
}

export function allowTopLevelReturn(): monaco.IDisposable {
	const oldCodesToIgnore = monaco.typescript.javascriptDefaults.getDiagnosticsOptions().diagnosticCodesToIgnore;
...
export function enableJavaScriptBrowserCompletion(): monaco.IDisposable {
	const oldLibs = monaco.typescript.javascriptDefaults.getCompilerOptions().lib;
...
export async function getJavaScriptWorker(model: monaco.editor.ITextModel): Promise<monaco.typescript.TypeScriptWorker | null> {
